...
import { SwapInfo, SwapProvider, generateSwapInfo, mergeSwapInfos } from "./swap/index";
import {
...
import MintifyModuleAbi from "./abis/MintifyModule.json";
...
import ZoraV4ModuleAbi from "./abis/ZoraV4Module.json";
import RelaySwapModuleAbi from "./abis/RelaySwapModule.json";
...
export class Router {
  ...
  constructor(chainId: number, provider: Provider, options?: SetupOptions) {
    ...
    this.contracts = {
      ...
      zoraV4Module: new Contract(
        Addresses.ZoraV4Module[chainId] ?? AddressZero,
        ZoraV4ModuleAbi,
        provider
      ),
      relaySwapModule: new Contract(
        Addresses.RelaySwapModule[chainId] ?? AddressZero,
        RelaySwapModuleAbi,
        provider
      ),
    };
  ...
  public async fillListingsTx(
    ...
    // We don't have a module for PaymentProcessorV21 listings
    if (details.some(({ kind }) => kind === "payment-processor-v2.1")) {
      const splittedDetails: ListingDetails[][] = [];

      const allPPv2Details = details.filter(({ kind }) => kind === "payment-processor-v2.1");
      ...
      for (const ppv2Details of splittedDetails) {
        const exchange = new Sdk.PaymentProcessorV21.Exchange(this.chainId);
        const operator = exchange.contract.address;

        const allFees = ppv2Details.map((c) => getFees([c]));
        const trustedChannel = ppv2Details[0].extraArgs?.trustedChannel;

        const orders: Sdk.PaymentProcessorV21.Order[] = ppv2Details.map(
          (c) => c.order as Sdk.PaymentProcessorV21.Order
        );
        ...
        for (const detail of ppv2Details) {
          const order = detail.order as Sdk.PaymentProcessorV21.Order;
          if (buyInCurrency !== detail.currency) {
            swapDetails.push({
              tokenIn: buyInCurrency,
              tokenOut: detail.currency,
        ...
        if (useSweepCollection) {
          txs.push({
            ...
            permits: [],
            txTags: {
              listings: { "payment-processor-v2.1": orders.length },
            },
            preSignatures: [],
            txData: exchange.sweepCollectionTx(taker, orders, {
              trustedChannel,
    ...
    if (blockedPaymentProcessorDetails.length) {
      ...
      for (const d of blockedPaymentProcessorDetails) {
        const order = d.order as Sdk.PaymentProcessor.Order;
        if (buyInCurrency !== d.currency) {
          swapDetails.push({
            tokenIn: buyInCurrency,
            tokenOut: d.currency,
            tokenOutAmount: bn(order.params.price)
              .div(order.params.amount)
              .mul(d.amount ?? 1),
            recipient: taker,
            refundTo: taker,
    ...
    if (blurCompatibleListings.length) {
      try {
        ...
        if (!options?.partial) {
          throw new Error(getErrorMessage(error, "Blur"));
        }
      }
    }
    ...
    let includesSeaportV16RoyaltyEnforcingOrders = false;
    await Promise.all(
      details.map(async (detail, i) => {
        if (["seaport-v1.5-partial", "seaport-v1.6-partial"].includes(detail.kind)) {
          ...
          let fulfiller = relayer;
          if (order.zone === Sdk.SeaportBase.Addresses.OpenSeaV16SignedZone[this.chainId]) {
            includesSeaportV16RoyaltyEnforcingOrders = true;
            ...
            fulfiller = this.contracts.seaportV16Module.address;
          }
          ...
          try {
            ...
            if (!options?.partial) {
              throw new Error(getErrorMessage(error, "Opensea"));
            }
          }
          ...
          try {
            ...
            if (!options?.partial) {
              throw new Error(getErrorMessage(error, "OKX"));
            }
          }
          ...
          try {
            ...
            if (!options?.partial) {
              throw new Error(getErrorMessage(error, "Element"));
            }
          }
        }
      })
    ...
    if (
      ...
      !options?.forceRouter &&
      !options?.usePermit &&
      (details.length === 1 ? true : options?.partial)
    ) {
    ...
    if (
      !includesSeaportV16RoyaltyEnforcingOrders &&
      details.length &&
      details.every(
        ({ kind, fees, currency, order }) =>
          kind === "seaport-v1.6" &&
    ...
    if (
      ...
      details.every(
        ({ kind, fees, currency, order }) =>
          ...
          currency === details[0].currency &&
          // All orders must have the same conduit
          (order as Sdk.Mintify.Order).params.conduitKey ===
            (details[0].order as Sdk.Mintify.Order).params.conduitKey &&
          !fees?.length
      ) &&
      !options?.forceRouter &&
      !options?.usePermit &&
      ...
      const exchange = new Sdk.Mintify.Exchange(this.chainId);

      const conduitKey =
        options?.conduitKey ?? (details[0].order as Sdk.Mintify.Order).params.conduitKey;
      const conduit = exchange.deriveConduit(conduitKey);
      ...
      if (details.length === 1) {
        const order = details[0].order as Sdk.Mintify.Order;
        return {
          txs: [
            {
              approvals: approval ? [approval] : [],
        ...
        const orders = details.map((d) => d.order as Sdk.Mintify.Order);
          ...
          txs: [
            {
              ...
              preSignatures: [],
              txTags: {
                listings: { mintify: details.length },
              },
              txData: await exchange.fillOrdersTx(
                relayer,
                orders,
    ...
    const zoraV4Details: ListingDetails[] = [];
    ...
    const mintifyDetails: PerCurrencyListingDetails = {};
    ...
    for (const detail of details) {
      ...
      switch (kind) {
        ...
        case "mintify":
          if (!mintifyDetails[currency]) {
            mintifyDetails[currency] = [];
          }
          detailsRef = mintifyDetails[currency];
          break;
    ...
    // Handle Mintify listings
    if (Object.keys(mintifyDetails).length) {
      const exchange = new Sdk.Mintify.Exchange(this.chainId);
      for (const currency of Object.keys(mintifyDetails)) {
        const rawCurrencyDetails = mintifyDetails[currency];

        // Account private vs non-private orders separately
        const privateDetails: Sdk.RouterV6.Types.ListingDetails[] = [];
        const normalDetails: Sdk.RouterV6.Types.ListingDetails[] = [];
        for (const currencyDetail of rawCurrencyDetails) {
          if ((currencyDetail.order as Sdk.Mintify.Order).isPrivateOrder()) {
            privateDetails.push(currencyDetail);
          } else {
            normalDetails.push(currencyDetail);
          }
        ...
        for (const currencyDetails of splittedDetails) {
          const orders = currencyDetails.map((d) => d.order as Sdk.Mintify.Order);
          const isPrivate = orders[0].isPrivateOrder();
          ...
          addRouterTags("mintify", orders.length, fees.length);
        ...
        try {
          ...
          if (options?.onError) {
            await options.onError("nftx-listing", error, {
              ...
              isUnrecoverable: ["INSUFFICIENT_LIQUIDITY", "MINT_FEE_EXCEEDS_VALUE"].includes(
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
          ...
          if (!options?.partial) {
            throw new Error(getErrorMessage(error, "NFTX"));
          }
        }
      }
    ...
    if (nftxV3Details.length) {
      ...
      for (const [vaultId, orders] of Object.entries(perVaultIdOrders)) {
        ...
        try {
          ...
          if (options?.onError) {
            const vaultDetails = nftxV3Details.filter(
              (c) => (c.order as Sdk.NftxV3.Order).params.vaultId === order.params.vaultId
            );
            for (const detail of vaultDetails) {
              options.onError("nftx-listing", error, {
                orderId: detail.orderId,
                additionalInfo: {
                  detail: detail,
                  taker,
    ...
    // Handle Zora V4 listings
    if (zoraV4Details.length) {
      const module = this.contracts.zoraV4Module;
      const aggregatedOrders: {
        pool: string;
        collection: string;
        tokenId: string;
      ...
      for (const details of zoraV4Details) {
        const order = details.order as Sdk.ZoraV4.Order;
        try {
          const { price } = await order.getQuote(5, this.provider);
          aggregatedOrders.push({
            pool: order.params.pool,
            collection: order.params.collection,
            tokenId: order.params.tokenId,
            amount: String(details.amount ?? 1),
            sqrtPriceLimitX96: 0,
            price,
          });
        } catch (error) {
          if (options?.onError) {
            options.onError("zora-v4-listing", error, {
              orderId: details.orderId,
              additionalInfo: {
                detail: details,
                taker,
              ...
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              isUnrecoverable: (error as any).toString().includes("call revert"),
            });
          }
          if (!options?.partial) {
            throw new Error(getErrorMessage(error, "ZORA-V4"));
          }
        }
      }

      // Consider the updated prices (fetched above from NFTX API)
      const fees = getFees(zoraV4Details);
      const price = aggregatedOrders.map((order) => order.price).reduce((a, b) => a.add(b), bn(0));
      const feeAmount = fees.map(({ amount }) => bn(amount)).reduce((a, b) => a.add(b), bn(0));
      const totalPrice = price.add(feeAmount);
      ...
      executions.push({
        ...
        orderIds: zoraV4Details.map((d) => d.orderId),
      });
      ...
      swapDetails.push({
        ...
        recipient: module.address,
        refundTo: relayer,
        details: zoraV4Details,
        executionIndex: executions.length - 1,
      });

      addRouterTags("zora-v4", zoraV4Details.length, fees.length);

      // Mark the listings as successfully handled
      for (const { orderId } of zoraV4Details) {
        success[orderId] = true;
      }
    }
    ...
    if (x2y2Details.length) {
      ...
      if (orders.length === 1) {
        try {
          ...
          if (!options?.partial) {
            throw new Error(getErrorMessage(error, "X2Y2"));
          }
        }
      } else {
        ...
        for (const [i, order] of orders.entries()) {
          ...
          if (cbId) {
            ...
            await exchange.releaseOrder(taker, order).catch(async (error) => {
              ...
              if (!options?.partial) {
                throw new Error(getErrorMessage(error, "ZeroExV4"));
              } else {
    ...
    if (swapDetails.length) {
      ...
      for (const swapDetails of Object.values(aggregatedSwapDetails)) {
        ...
        const swapModule =
          swapProvider === "uniswap" ? this.contracts.swapModule : this.contracts.relaySwapModule;
  ...
  public async fillBidsTx(
    ...
    const success: { [orderIdAndTokenId: string]: boolean } = {};
      ...
      try {
        ...
        for (const [contract, data] of Object.entries(result)) {
          ...
          if (successfulBlurDetails.length) {
            ...
            for (const d of successfulBlurDetails) {
              success[`${d.orderId}:${d.tokenId}`] = true;
              orderIds.push(d.orderId);
            }
        ...
        if (options?.onError) {
          for (const detail of blurDetails) {
            if (!success[`${detail.orderId}:${detail.tokenId}`]) {
    ...
    if (blurDetails.find((d) => !success[`${d.orderId}:${d.tokenId}`])) {
      if (!options?.partial) {
        throw new Error("Could not fetch calldata for all Blur bids");
      }
    }

    // If all orders are Mintify ones, fill directly via their exchange contract
    if (!options?.forceApprovalProxy && details.every((d) => d.kind === "mintify")) {
      ...
      for (let i = 0; i < details.length; i++) {
        ...
        batch.orders.push(
          ...
          },
          {
            ...
            extraData: "0x",
            numerator: String(detail.amount ?? 1),
            denominator: String(detail.amount ?? 1),
          }
        );

        // All `orderIndex` fields must be adjusted
        batch.criteriaResolvers.push(
          ...(matchParams.criteriaResolvers ?? []).map((cr) => ({
            ...cr,
        ...
        success[`${detail.orderId}:${detail.tokenId}`] = true;
      }
      ...
      txs.push({
        txTags: {
          bids: { mintify: details.length },
        },
        txData: {
          from: taker,
          to: Sdk.Mintify.Addresses.Exchange[this.chainId],
          data: calldata + generateSourceBytes(options?.source),
          gas: this.chainId === Network.Flow ? String(1000000 * details.length) : undefined,
        },
        // Ensure approvals are unique
        approvals: uniqBy(
        ...
        ftApprovals: [],
        preSignatures: [],
        orderIds: [...new Set(details.map((detail) => detail.orderId))],
      });
    }

    // Handle signature retrieval for Element orders
    await Promise.all(
      details.map(async (detail, i) => {
    ...
    if (details.some(({ kind }) => kind === "payment-processor-v2")) {
      ...
      for (const ppv2Details of splittedDetails) {
        ...
        txs.push({
          ...
          txData: exchange.fillOrdersTx(
            ...
            orders.map((_, i) => {
              return {
                taker,
                tokenId: ppv2Details[i].tokenId,
                amount: ppv2Details[i].amount ?? 1,
                ...(ppv2Details[i].extraArgs ?? {}),
              };
            }),
            {
              trustedChannel,
        ...
        for (const { orderId, tokenId } of ppv2Details) {
          success[`${orderId}:${tokenId}`] = true;
        }
      }
    }

    // Fill PaymentProcessorV21 offers directly
    if (details.some(({ kind }) => kind === "payment-processor-v2.1")) {
      const splittedDetails: BidDetails[][] = [];
    ...
    for (let i = 0; i < details.length; i++) {
      ...
      switch (detail.kind) {
        ...
        case "mintify": {
          module = this.contracts.mintifyModule;
          break;
        }
    ...
    for (let i = 0; i < details.length; i++) {
      ...
      switch (detail.kind) {
        ...
        case "seaport-v1.6": {
          ...
          if (
            order.params.zone ===
            Sdk.SeaportBase.Addresses.ReservoirV16RoyaltyEnforcingZone[this.chainId]
          ) {
            protectedSeaportV16Offers.push({
              detail,
              order,
              fees,
              criteriaResolvers: matchParams.criteriaResolvers ?? [],
              extraData: await exchange.getExtraData(order, matchParams),
            });
          } else {
            executionsWithDetails.push({
        ...
        case "nftx-v3": {
          ...
          try {
            ...
            if (options?.onError) {
              options.onError("nftx-v3-offer", error, {
                orderId: detail.orderId,
                isUnrecoverable: ["INSUFFICIENT_LIQUIDITY", "MINT_FEE_EXCEEDS_VALUE"].includes(
                  // eslint-disable-next-line @typescript-eslint/no-explicit-any
                  (error as any).response?.data?.code
        ...
        case "zora-v4": {
          const order = detail.order as Sdk.ZoraV4.Order;
          const module = this.contracts.zoraV4Module;
          ...
          try {
            ...
            if (options?.onError) {
              options.onError("zora-v4-offer", error, {
                orderId: detail.orderId,
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                isUnrecoverable: (error as any).toString().includes("call revert"),
                additionalInfo: {
      ...
      if (
        sellOutCurrency &&
        success[`${detail.orderId}:${detail.tokenId}`] &&
        detail.currency !== sellOutCurrency
      ) {
    ...
    const detailsWithPermits = details.filter(
      (d) => d.permit && success[`${d.orderId}:${d.tokenId}`]
    );
      ...
      if (detail.contractKind === "erc721") {
        txs.push({
          txData: {
            ...
            gas: this.chainId === Network.Flow ? String(1000000) : undefined,
          },
          txTags: routerTxTags,
          approvals: [],
          ftApprovals: [],
  ...
  public async transfersTx(
    ...
    const useReservoirApprovalProxy = !useOpenseaTransferHelper
      ? Addresses.ApprovalProxy[this.chainId] &&
        conduitKey &&
        (await conduitController.getChannelStatus(
          conduitController.deriveConduit(conduitKey),
          Addresses.ApprovalProxy[this.chainId]
        ))
      : false;

    // Use native transfer if neither opensea nor reservoir are not supported
    if (useDirectTransfers || (!useOpenseaTransferHelper && !useReservoirApprovalProxy)) {
