...
export class Order {
  ...
  public isBulkOrder() {
    return this.params.bulkOrderProof !== undefined;
  }
  ...
  public getSellerPermitSignature() {
    return {
      v: 0,
      r: HashZero,
      s: HashZero,
  ...
  public getPermitContext() {
    return {
      permitProcessor: AddressZero,
      permitNonce: "0",
    };
  }
  ...
  public getMatchedOrder(
    ...
    return {
      ...
      requestedFillAmount: options?.amount ? options.amount.toString() : "1",
      minimumFillAmount: options?.amount ? options.amount.toString() : "1",
      signature: {
        r: this.params.r!,
        s: this.params.s!,
        v: this.params.v!,
  ...
  public checkSignature() {
    ...
    if (this.isBulkOrder()) {
      const bulkOrderProof = this.params.bulkOrderProof!;
      const key = Number(bulkOrderProof.orderIndex);
      const height = bulkOrderProof.proof.length;

      const proofElements: string[] = bulkOrderProof.proof;
      ...
      const types = { ...this.getEip712TypesAndValue()[0] };
      ...
      const bulkOrderTypeHash = solidityKeccak256(
        ["string"],
        [encoder.encodeType(`Bulk${originalType}`)]
      );
      ...
      if (lc(this.params.sellerOrBuyer) !== lc(signer)) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  public getEip712TypesAndValue(): any {
