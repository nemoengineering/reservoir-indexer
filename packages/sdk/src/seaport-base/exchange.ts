...
import { Network, TxData, bn, generateSourceBytes, lc, n, s } from "../utils";
            ...
            this.contract.interface.encodeFunctionData("matchAdvancedOrders", [
              [
                advancedOrder,
                {
                  ...counterOrder,
                  numerator: matchParams.amount ?? 1,
                  denominator: matchParams.amount ?? 1,
                  extraData: "0x",
                },
              ],
              [],
          ...
          value:
            info.paymentToken === CommonAddresses.Native[this.chainId]
              ? bn(order.getMatchingPrice(options?.timestampOverride))
                  ...
                  .toHexString()
              : undefined,
          gas: this.chainId === Network.Nebula ? String(200000 + 175000 * 1) : undefined,
        };
      }
  ...
  public async fillOrdersTx(
    ...
    if (containsPrivateOrders) {
      ...
      for (let i = 0; i < fillOrders.length; i++) {
        ...
        if (!order.isPrivateOrder()) {
          ...
          allAdvancedOrders.push({
            ...counterOrder,
            numerator: match.amount?.toString() ?? "1",
            denominator: match.amount?.toString() ?? "1",
            extraData: "0x",
          });
      ...
      return {
        ...
        gas: this.chainId === Network.Nebula ? String(200000 + 175000 * orders.length) : undefined,
      };
    }
