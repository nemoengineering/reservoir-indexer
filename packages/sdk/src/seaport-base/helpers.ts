...
export const isCosignedOrder = (params: Types.OrderComponents, chainId: number) =>
  ...
  [
    ...
    BaseAddresses.ReservoirV16RoyaltyEnforcingZone[chainId],
...
export const constructOfferCounterOrderAndFulfillments = (
  ...
  const orderQuantity = params.consideration[0].startAmount;
  const quantity = details.amount ? bn(details.amount).toString() : orderQuantity;
  ...
  const fillAmount = quantity ?? "1";
  const pricePerQuantity = netAmount.div(orderQuantity);
  const fillQuantityNetAmount = pricePerQuantity.mul(quantity);
    ...
    offer: [
      {
        ...
        startAmount: fillAmount,
        endAmount: fillAmount,
      },
    ],
    consideration: [
      {
        ...
        startAmount: fillQuantityNetAmount.sub(totalTips).toString(),
        endAmount: fillQuantityNetAmount.sub(totalTips).toString(),
        recipient: taker,
      },
      ...details.tips.map((t) => ({
        itemType: params.offer[0].itemType,
...
export const constructListingCounterOrderAndFulfillments = (
  ...
  const { aggregatedStartAmount } = paymentItems.reduce(
    ({ aggregatedStartAmount, aggregatedEndAmount }, item) => ({
      aggregatedStartAmount: aggregatedStartAmount.add(item.startAmount),
      aggregatedEndAmount: aggregatedEndAmount.add(item.endAmount),
    }),
  ...
  const fillAmount = matchAmount ?? "1";
  const listingAmount = params.offer[0].startAmount;
  const pricePerQuantity = aggregatedStartAmount.div(listingAmount);
  const fillPrice = pricePerQuantity.mul(bn(fillAmount));
      ...
      offer: [
        {
          ...
          token: paymentItems[0].token,
          identifierOrCriteria: paymentItems[0].identifierOrCriteria,
          startAmount: fillPrice.toString(),
          endAmount: fillPrice.toString(),
        },
      ],
