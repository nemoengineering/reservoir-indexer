...
/**
 * @title  ZkSignedZoneController
 * @author BCLeFevre
 * @dev    This is a zk-compatible version of SignedZoneController. It uses
 *         the SignedZone runtime codehash to derive the zone address.
 ...
 */
contract ZkSignedZoneController is
  ...
  SignedZoneControllerEventsAndErrors
{
  ...
  // Set the signed zone runtime code hash as an immutable argument.
  ...
  constructor() {
    ...
    SignedZone zeroSignedZone = new SignedZone{salt: bytes32(0)}();
  ...
  function createZone(
    ...
    address signedZoneAddress = address(new SignedZone{salt: salt}());
  ...
  function getZone(bytes32 salt) external view override returns (address derivedAddress) {
    // Derive the SignedZone address from deployer, salt and runtime code
    // hash.
    bytes32 hash = keccak256(
      bytes.concat(
        keccak256("zksyncCreate2"),
