...
/**
 * @title  SignedZoneControllerV16Royalty
 * @author BCLeFevre
 ...
 */
contract SignedZoneControllerV16Royalty is
  ...
  SignedZoneControllerV16RoyaltyEventsAndErrors
{
  ...
  function createZone(
    ...
    bytes memory _SIGNED_ZONE_CREATION_CODE = abi.encodePacked(
      type(SignedZoneV16Royalty).creationCode,
      abi.encode(zoneName)
    );
  ...
  function getZone(
    ...
    bytes32 _SIGNED_ZONE_CREATION_CODE_HASH = keccak256(
      abi.encodePacked(type(SignedZoneV16Royalty).creationCode, abi.encode(zoneName))
    );
