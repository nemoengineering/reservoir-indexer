...
export const trigger = {
    ...
    PermitProxy: async (chainId: number) =>
      dv("PermitProxy", "v5", [
        ...
        [
          ...
          Sdk.Common.Addresses.GelatoRelay1BalanceConcurrentERC2771[chainId],
        ],
      ]),
  ...
  Modules: {
    ...
    ZoraV4Module: async (chainId: number) =>
      dv("ZoraV4Module", "v2", [
        DEPLOYER,
        Sdk.RouterV6.Addresses.Router[chainId],
    ...
    RelaySwapModule: async (chainId: number) =>
      dv("RelaySwapModule", "v2", [
        DEPLOYER,
        Sdk.RouterV6.Addresses.Router[chainId],
        Sdk.Common.Addresses.WNative[chainId],
      ]),
  ...
  Utilities: {
    PPV2TrustedForwarder: async (chainId: number) => {
      ...
      const factory = new Contract(
        Sdk.PaymentProcessorBase.Addresses.TrustedForwarderFactory[chainId],
        new Interface([
    ...
    OffChainCancellationZone: async (chainId: number, seaportVersion: "v1.5" | "v1.6") => {
      if (seaportVersion === "v1.5" && !Sdk.SeaportV15.Addresses.Exchange[chainId]) {
        console.log(
          "Skipping deploying OffChainCancellationZone for Seaport v1.5 due to missing Seaport deployment"
        );
        return;
      }

      if (seaportVersion === "v1.6" && !Sdk.SeaportV16.Addresses.Exchange[chainId]) {
        console.log(
          "Skipping deploying OffChainCancellationZone for Seaport v1.6 due to missing Seaport deployment"
        );
        return;
      }
    ...
    RoyaltyEnforcingZone: async (chainId: number) => {
      const version = "v1";

      if (!(await readDeployment("SignedZoneControllerV16Royalty", version, chainId))) {
        await dv("SignedZoneControllerV16Royalty", version, []);
      }
      await verify("SignedZoneControllerV16Royalty", version, []);

      const [deployer] = await ethers.getSigners();

      const controller = new Contract(
        await readDeployment("SignedZoneControllerV16Royalty", version, chainId).then((a) => a!),
        new Interface([
          "function createZone(string zoneName, string apiEndpoint, string documentationURI, address initialOwner, bytes32 salt)",
          "function getZone(string zoneName, bytes32 salt) view returns (address)",
          "function getActiveSigners(address zone) view returns (address[])",
          "function updateSigner(address zone, address signer, bool active)",
        ]),
        deployer
      ...
      const zoneName = "RoyaltyEnforcement";
      const zoneAddress = await controller
        .getZone(zoneName, salt)
        .then((a: string) => a.toLowerCase());
      ...
      if (code === "0x") {
        ...
        await writeDeployment(zoneAddress, "SignedZoneV16Royalty", version, chainId);
        await waitBeforeVerification();
      }
      await verify("SignedZoneV16Royalty", version, [zoneName]);
