...
export const setupRouterWithModules = async (chainId: number, deployer: SignerWithAddress) => {
  ...
  const zoraV4Module = await ethers
    .getContractFactory("ZoraV4Module", deployer)
    .then((factory) =>
      factory.deploy(router.address, deployer.address, Sdk.ZoraV4.Addresses.SecondarySwap[chainId])
    );
  Sdk.RouterV6.Addresses.ZoraV4Module[chainId] = zoraV4Module.address.toLowerCase();
