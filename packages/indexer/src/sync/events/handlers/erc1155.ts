...
export const handleEvents = async (events: EnhancedEvent[], onChainData: OnChainData) => {
  ...
  for (const { subKind, baseEventParams, log } of events) {
    ...
    if (skipContracts.includes(baseEventParams.address)) {
      continue;
    }
  ...
  for (const [txHash, mints] of mintedTokens.entries()) {
    if (mints.length > 0) {
      ...
      const batchIndexCounter: Map<string, number> = new Map();
      ...
      for (const mint of mints) {
        ...
        // Handle batch index
        const batchIndexCounterKey = `${mint.baseEventParams.txHash}:${mint.baseEventParams.logIndex}`;
        if (batchIndexCounter.has(batchIndexCounterKey)) {
          let currentCounter = Number(batchIndexCounter.get(batchIndexCounterKey));
          batchIndexCounter.set(batchIndexCounterKey, ++currentCounter);
        } else {
          batchIndexCounter.set(batchIndexCounterKey, 1);
        }
        ...
        onChainData.fillEvents.push({
          ...
          isPrimary: true,
          baseEventParams: {
            ...mint.baseEventParams,
            batchIndex: batchIndexCounter.get(batchIndexCounterKey) ?? 1,
          },
        });
      }
    }
