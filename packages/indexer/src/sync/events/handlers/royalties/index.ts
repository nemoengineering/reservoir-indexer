...
export const assignRoyaltiesToFillEvents = async (
  ...
  for (const fillEventsChunk of _.chunk(fillEvents, 25)) {
    const mintFillEventsChunk = fillEventsChunk.filter(
      (fillEvent) => fillEvent.orderKind === "mint"
    );

    if (fillEventsChunk.length === mintFillEventsChunk.length) {
      continue;
    }
        ...
        limit(async () => {
          ...
          try {
            ...
            logger.error(
              ...
              JSON.stringify({
                error,
                fillEventsLength: fillEvents.length,
                fillEventsChunkLength: fillEventsChunk.length,
                fillEvent,
                retry: (error as any).response?.status === 429,
              })
            );
          }
        })
