...
export const addEvents = async (events: Event[], backfill: boolean) => {
  ...
  for (const transferValuesChunk of _.chunk(transferValues, 50)) {
    const queries: string[] = [];

    if (transferValuesChunk.length) {
      ...
      queries.push(`
        WITH "x" AS (
          INSERT INTO "ft_transfer_events" (
            ...
            "to",
            "amount"
          ) VALUES ${pgp.helpers.values(transferValuesChunk, columns)}
          ON CONFLICT DO NOTHING
          RETURNING
            "address",
            ARRAY["from", "to"] AS "owners",
            ARRAY[GREATEST(-"amount", -1e77+1), LEAST("amount", 1e78-1)] AS "amount_deltas"
        )
        ...
        UPDATE SET "amount" = GREATEST(LEAST("ft_balances"."amount" + "excluded"."amount", 1e78-1), -1e77+1), updated_at = now()
      `);
    }
    ...
    if (queries.length) {
      if (backfill) {
        ...
        await eventsSyncFtTransfersWriteBufferJob.addToQueue({
