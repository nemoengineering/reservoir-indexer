...
import amqplibConnectionManager, { ChannelWrapper } from "amqp-connection-manager";
...
export class RabbitMq {
  public static delayedExchangeName = `${getChainName()}.delayed`;
  private static maxPublisherChannelsCount = 10;
  private static rabbitMqPublisherChannels: ChannelWrapper[] = [];
  private static rabbitMqPublisherChannelsBackfill: ChannelWrapper[] = [];

  public static hashKey = "rabbit_assert_queues_exchanges_hash";

  public static async connect() {
    await RabbitMq.openConnections();
    if (config.rabbitHostnameBackfill) {
      await RabbitMq.openConnections("backfill");
    }
  }

  public static async openConnections(type: "" | "backfill" = "") {
    const rabbitMqPublisherConnection = amqplibConnectionManager.connect(
      type === "backfill"
        ? {
    ...
    let rabbitMqPublisherChannels = RabbitMq.rabbitMqPublisherChannels;
    if (type === "backfill") {
      rabbitMqPublisherChannels = RabbitMq.rabbitMqPublisherChannelsBackfill;
    }

    for (let index = 0; index < RabbitMq.maxPublisherChannelsCount; ++index) {
      const channel = rabbitMqPublisherConnection.createChannel();
      await channel.waitForConnect();
      rabbitMqPublisherChannels[index] = channel;
    ...
    rabbitMqPublisherConnection.once("error", (error) => {
      logger.error("rabbit-connection", `Publisher connection error ${error}`);
    });

    rabbitMqPublisherConnection.once("close", (error) => {
  ...
  public static async send(
    queueName: string,
    queue: AbstractRabbitMqJobHandler,
    ...
    try {
      if (delay) {
        ...
        await rabbitMqPublisherChannels[channelIndex].publish(
        ...
        await rabbitMqPublisherChannels[channelIndex].sendToQueue(
          queueName,
          Buffer.from(JSON.stringify(content)),
          {
            persistent: content.persistent,
    ...
    await Promise.all(
      messages.map((message) =>
        limit(() => {
          ...
          return RabbitMq.send(queueName, queue, message.content, message.delay, message.priority);
        })
      )
    );
  }
  ...
  public static async createOrUpdatePolicy(policy: CreatePolicyPayload) {
    const url = `${config.rabbitHttpUrl}/api/policies/${getChainName()}/${policy.name}`;
  ...
  public static async createVhost() {
    if (config.assertRabbitVhost) {
      const url = `${config.rabbitHttpUrl}/api/vhosts/${getChainName()}`;
      await axios.put(url);
    }

    // For backfill cluster make sure vhost exist
    if (config.rabbitHostnameBackfill) {
      const url = `${config.rabbitHttpUrlBackfill}/api/vhosts/${getChainName()}`;
      await axios.put(url);
    }
  }
  ...
  public static async assertQueuesAndExchanges(type: "" | "backfill" = "") {
    const abstract = await import("@/jobs/abstract-rabbit-mq-job-handler");
    const jobsIndex = await import("@/jobs/index");
    ...
    const processedQueuesNames: string[] = [];

    for (const queue of consumerQueues) {
      // Make sure all queues names are unique
      if (processedQueuesNames.includes(queue.getQueue())) {
        throw new Error(`Duplicate queue name ${queue.getQueue()}`);
      }

      // If backfill assert only queues using the backfill
      if (type == "backfill" && !queue.isUsingBackfillRabbit()) {
        continue;
      }

      processedQueuesNames.push(queue.getQueue());
