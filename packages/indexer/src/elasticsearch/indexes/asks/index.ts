...
const INDEX_NAME = config.elasticsearchAsksIndexName || `asks`;
...
export const getAttributeFloorSellValueAndOnSaleCount = async (
  collection: string,
  key: string,
  value: string
): Promise<{
  ...
  (esQuery as any).bool = {
    filter: [
      ...
      {
        term: {
          [`token.attributesV2.${key}`]: value,
        },
      },
    ],
  };

  (esAggs as any).uniqueSales = {
    cardinality: {
      field: `token.id`,
    },
  };
  ...
  const asks: AskDocument[] = esResult.hits.hits.map((hit) => hit._source! as AskDocument);

  if (asks.length) {
    return {
      floorSell: {
        id: asks[0].order.id,
        value: asks[0].order.pricing.value!,
        currency: asks[0].order.pricing.currency,
        currencyValue: asks[0].order.pricing.currencyValue!,
        maker: asks[0].order.maker,
        validFrom: asks[0].order.validFrom,
        validTo: asks[0].order.validUntil,
        sourceIdInt: asks[0].order.sourceId,
      },
      onSaleCount: (esResult?.aggregations?.uniqueSales as any).value,
    };
  }
...
export const searchTokenAsks = async (
  params: {
    ...
    sources?: { operation: "include" | "exclude"; ids: number[] };
    limit?: number;
  ...
  if (params.sources?.ids.length) {
    if (params.sources?.operation === "exclude") {
      (esQuery as any).bool.must_not.push({
        terms: { "order.sourceId": params.sources.ids },
      });
    } else {
      (esQuery as any).bool.filter.push({
        terms: { "order.sourceId": params.sources.ids },
...
export const updateAsksTokenData = async (
  ...
  try {
    ...
    const pendingUpdateDocuments: { id: string | undefined; index: string }[] =
      esResult.hits.hits.map((hit) => ({ id: hit._id, index: hit._index }));
