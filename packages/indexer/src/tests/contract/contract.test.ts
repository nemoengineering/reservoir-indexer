...
describe("Contract", () => {
  it("check-contract-has-staking-by-keywords", async () => {
    ...
    for (const skipCombination of skipCombinations) {
      ...
      const staked = await checkContractHasStakingKeywords(collection);
      expect(staked).toBe(true);
    }
  });

  it("polygon-proxy-issue", async () => {
    const staked = await checkContractHasStakingKeywords(
      "0x670fd103b1a08628e9557cd66b87ded841115190"
    );
    expect(staked).toBe(true);
  });

  it("mainnet-proxy-issue", async () => {
    const staked = await checkContractHasStakingKeywords(
      "0x823460fd74d322b57cd07562c25c2f01376c71a1"
    );
    expect(staked).toBe(true);
  });
});
