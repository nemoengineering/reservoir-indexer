...
export const getAttributesAllV4Options: RouteOptions = {
  ...
  handler: async (request: Request) => {
    ...
    const baseQuery = `
        ...
        AND attribute_keys.kind = 'number'
      ...
      return result.map(async (r) => {
        ...
        if (r.kind == "number") {
          return {
            key: r.key,
            kind: r.kind,
            minRange: _.isArray(r.values) ? Number((_.first(r.values) as any)["min_range"]) : null,
            maxRange: _.isArray(r.values) ? Number((_.first(r.values) as any)["max_range"]) : null,
          };
        } else {
          return {
            key: r.key,
