...
export const getTrendingMintsV2Options: RouteOptions = {
  cache: {
    expiresIn: 60 * 1000,
    privacy: "public",
  },
...
async function getMintStages(contracts: string[]): Promise<Record<string, Mint["mint_stages"]>> {
  const baseQuery = `
    ...
    WHERE collection_mints.collection_id IN (${contracts
      ...
      .join(",")})
    GROUP BY collection_mints.collection_id`;
...
async function formatCollections(
  ...
  const collections = await Promise.all(
    collectionsResult.map(async (r) => {
      ...
      const mintPrice = mintType === "free" ? "0" : mintData?.price;
      ...
      return {
        ...
        contractKind: metadata.contract_kind,
        tokenCount: String(metadata.token_count || 0),
        ...
        mintPrice: mintPrice
          ? await getJoiPriceObject(
              { gross: { amount: mintPrice } },
              mintData.currency ?? Sdk.Common.Addresses.Native[config.chainId]
            )
          : null,
