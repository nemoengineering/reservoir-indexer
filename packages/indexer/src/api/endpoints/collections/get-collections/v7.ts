  ...
  validate: {
    query: Joi.object({
      ...
      limit: Joi.number()
        ...
        .when("sortBy", {
          switch: [
            { is: Joi.string().valid("updatedAt"), then: Joi.number().integer().max(1000) },
            {
              is: Joi.string().valid("lastMintTimestamp"),
  ...
  handler: async (request: Request) => {
    ...
    let baseQuery = `
        SELECT
          ...
          collections.last_mint_timestamp,
    ...
    switch (query.sortBy) {
      case "lastMintTimestamp": {
        if (query.continuation) {
          query.contParam = query.contParam === "null" ? 0 : query.contParam;
          conditions.push(
            `(COALESCE(collections.last_mint_timestamp, 0), collections.id) ${sign} ($/contParam/, $/contId/)`
          );
        }
        orderBy = ` ORDER BY collections.last_mint_timestamp ${query.sortDirection} NULLS LAST, collections.id ${query.sortDirection}`;
    ...
    query._limit = query.limit + 1;

    const results = await redb.manyOrNone(baseQuery, query);

    // Pagination
    const hasContinuation = results.length >= query._limit;
        ...
        switch (query.sortBy) {
          case "lastMintTimestamp": {
            continuation = buildContinuation(
              `${lastCollection.last_mint_timestamp}_${lastCollection.id}`
            );
            break;
          }
