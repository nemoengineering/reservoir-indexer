...
export async function getCollectionsMetadata(
  ...
  const collectionsToFetch = collectionIds.map((id: string) => `collection-cache:v8:${id}`);
  ...
  `;
      ...
      return getJoiCollectionObject(
        {
          ...
          creator: creator ? fromBuffer(creator) : null,
        },
        metadata_disabled
      );
    });
    ...
    const commands = flatMap(collectionMetadataResponse, (metadata: any) => {
      return [
        ["set", `collection-cache:v8:${metadata.id}`, JSON.stringify(metadata)],
        ["expire", `collection-cache:v8:${metadata.id}`, REDIS_EXPIRATION],
      ];
    });
