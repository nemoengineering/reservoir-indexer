...
import { isExchangeEnabled, OrderKind } from "@/orderbook/orders";
...
export const getCollectionMarketplaceConfigurationsV2Options: RouteOptions = {
  ...
  handler: async (request: Request) => {
    ...
    try {
        ...
        marketplaces.push({
          ...
          exchanges: {
            ...
            "payment-processor-v2.1": {
              orderKind: "payment-processor-v2.1",
              enabled: true,
              customFeesSupported: true,
              numFeesSupported: 1,
              collectionBidSupported:
      ...
      // Handle OpenSea
      {
        const openseaMarketplaceFees: Royalty[] = marketplaceFees.getCollectionOpenseaFees();
      ...
      for await (const marketplace of marketplaces) {
        ...
        await Promise.allSettled(
          Object.values(marketplace.exchanges).map(async (exchange) => {
            exchange.enabled =
              exchange.enabled &&
              Boolean(marketplace.orderbook && supportedOrderbooks.includes(marketplace.orderbook));
            ...
            if (exchange.enabled) {
              ...
              const defaultSeaportConduit = new Sdk.SeaportBase.ConduitController(
                config.chainId
              ).deriveConduit(
                Sdk.SeaportBase.Addresses.OpenseaConduitKey[config.chainId] ??
              ...
              switch (exchange.orderKind) {
                ...
                case "mintify": {
                  operators = [
                    Sdk.Mintify.Addresses.Exchange[config.chainId],
                    defaultSeaportConduit,
                  ];
                  break;
                }
                ...
                case "payment-processor-v2.1": {
                  operators = [Sdk.PaymentProcessorV21.Addresses.Exchange[config.chainId]];
                  break;
                }
              }

              const result = await isExchangeEnabled(contract, {
                operators,
                orderKind: exchange.orderKind!,
                orderbook: marketplace.orderbook!,
              });
              exchange.enabled = result.enabled;
              exchange.royaltiesEnforced = result.royaltiesEnforced;

              if (exchange.orderKind === "payment-processor") {
              ...
              } else if (exchange.orderKind === "payment-processor-v2") {
                const settings = await paymentProcessorV2.getConfigByContract(contract);

                let paymentTokens: string[] | undefined = [
                ...
                if (
                  ...
                  settings?.paymentSettings ===
                  paymentProcessorV2.PaymentSettings.AllowAnyPaymentMethod
                ) {
                  paymentTokens = undefined;
                }

                exchange.paymentTokens = paymentTokens
                  ? await Promise.all(
              ...
              } else if (exchange.orderKind === "payment-processor-v2.1") {
                const settings = await paymentProcessorRegistry.getConfigByContract(
                ...
                if (
                  settings &&
                  [
                    paymentProcessorRegistry.PaymentSettings.Default,
                    paymentProcessorRegistry.PaymentSettings.Custom,
                  ].includes(settings.paymentSettings)
                ) {
                  paymentTokens = settings.whitelistedPaymentMethods;
                } else if (
                  settings &&
                  [
                    paymentProcessorRegistry.PaymentSettings.PricingConstraints,
                    paymentProcessorRegistry.PaymentSettings.PricingConstraintsCollectionOnly,
                  ].includes(settings.paymentSettings)
                ) {
                  paymentTokens = [settings.constrainedPricingPaymentMethod];
                  exchange.maxPriceRaw = settings?.pricingBounds?.ceilingPrice;
                  ...
                  settings?.paymentSettings === paymentProcessorRegistry.PaymentSettings.Any
                ) {
                  paymentTokens = undefined;
                } else if (
                  settings?.paymentSettings === paymentProcessorRegistry.PaymentSettings.Paused
                ) {
