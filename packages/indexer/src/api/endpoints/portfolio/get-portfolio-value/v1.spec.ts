...
import { getPortfolioValueV1Options } from "./v1";
...
describe("Get portfolio value", () => {
  ...
  it("can get portfolio for v2 and v3 tokens", async () => {
    ...
    const contracts = tokens.map((token: any) => fromBuffer(token.contract));
    const usdPrices = await getUsdPricesUniswap(contracts);
    const keys = Object.keys(usdPrices);
    expect(contracts.sort()).toEqual(keys.sort());
    keys.map((key) => {
      expect(usdPrices[key]?.priceUSD).toBeTruthy();
    });

    const result = await (getPortfolioValueV1Options as any).handler({
      query: { wallet: wallet.toLowerCase() },
    });
    expect(result.portfolio).toBeDefined();
    expect(result.portfolio.totalUsdValue).toBeDefined();
    expect(result.portfolio.totalUsdValue).toBeTruthy();
  });
});
