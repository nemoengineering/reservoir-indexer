...
export const getCollectionTopBidOracleV3Options: RouteOptions = {
  ...
  handler: async (request: Request) => {
    ...
    try {
      ...
      if (Sdk.Common.Addresses.Native[config.chainId] === query.currency) {
        // ETH: do nothing
      } else if (Sdk.Common.Addresses.WNative[config.chainId] === query.currency) {
        // WETH: do nothing
      } else if (
        [
          Object.values(Sdk.Common.Addresses.Usdc[config.chainId]).flat(),
