...
export const postExecuteTransferV1Options: RouteOptions = {
  ...
  handler: async (request: Request) => {
    ...
    try {
      ...
      // If any contract is enforcing royalties, then use direct transfers for everything
      let useDirectTransfers: boolean | undefined;
      for (const item of transferItem.items) {
        const erc721cConfigV3 = await erc721c.v3.getConfigFromDb(item.token);
        if (
          erc721cConfigV3 &&
