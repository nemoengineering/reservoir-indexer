  ...
  validate: {
    payload: Joi.object({
      ...
      maxPriorityFeePerGas: Joi.string()
        .pattern(regex.number)
        .description("Optional. Set custom gas price"),
      forceOnChainCancel: Joi.boolean().default(false),
    })
      .oxor("orderIds", "token")
  ...
  handler: async (request: Request) => {
    ...
    try {
      ...
      for (const order of orderResults) {
        ...
        if (
          offchainCancel.isOrderNativeOffChainCancellable(order.raw_data) &&
          !payload.forceOnChainCancel &&
          !(await redis.get(`cosigner-order:${order.id}`))
        ) {
      ...
      for (const [kind, data] of Object.entries(ordersByKind)) {
        ...
        if (data.offchainCancellable.length) {
          ...
          if (kind === "payment-processor-v2") {
          ...
          } else if (kind === "payment-processor-v2.1") {
            steps[1].items.push({
              status: "incomplete",
              orderIds,
              data: {
            ...
            steps[1].items.push({
              ...
              orderIds,
              data: {
                sign: await offchainCancel.seaport.generateOffChainCancellationSignatureData(
                  orderIds
                ),
                post: {
                  endpoint: "/execute/cancel-signature/v1",
        ...
        if (data.onchainCancellable.length) {
          ...
          switch (kind as OrderKind) {
            ...
            case "payment-processor-v2.1": {
              for (const order of data.onchainCancellable) {
                const sdkOrder = new Sdk.PaymentProcessorV21.Order(config.chainId, order.raw_data);
                const exchange = new Sdk.PaymentProcessorV21.Exchange(config.chainId);
