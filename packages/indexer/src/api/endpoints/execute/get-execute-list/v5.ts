...
import * as paymentProcessorV21SellToken from "@/orderbook/orders/payment-processor-v2.1/build/sell/token";
    ...
    try {
      ...
      await Promise.all(
        params.map(async (params, i) => {
          ...
          try {
            // Blacklist checks
            await checkBlacklistAndFallback(contract, params, key);
          } catch (error) {
          ...
          try {
            switch (params.orderKind) {
                ...
                steps[2].items.push({
                  ...
                  data: {
                    sign: {
                      ...
                      value: {
                        ...signData.value,
                        salt: bn(signData.value.salt).toString(),
                        nonce: bn(signData.value.nonce).toString(),
                      },
              ...
              case "payment-processor-v2.1": {
                ...
                const order = await paymentProcessorV21SellToken.build({
                  ...params,
                  ...options,
                  maker,
                  contract,
