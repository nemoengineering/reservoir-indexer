...
// PaymentProcessorV2.1
import * as paymentProcessorV21BuyToken from "@/orderbook/orders/payment-processor-v2.1/build/buy/token";
import * as paymentProcessorV21BuyCollection from "@/orderbook/orders/payment-processor-v2.1/build/buy/collection";
import * as paymentProcessorV21BuyAttribute from "@/orderbook/orders/payment-processor-v2.1/build/buy/attribute";
...
export const getExecuteBidV5Options: RouteOptions = {
        ...
        .items(
          Joi.object({
            ...
            options: Joi.object({
              ...
              "payment-processor-v2.1": Joi.object({
                useOffChainCancellation: Joi.boolean().required(),
                cosigner: Joi.string().lowercase().optional(),
                replaceOrderId: Joi.string().when("useOffChainCancellation", {
                  is: true,
  ...
  handler: async (request: Request) => {
    ...
    try {
      ...
      if (["0xb65e9d4fb5e89c18b8fe77cc47f28b6c2c7b4aac"].includes(maker)) {
      ...
      const bulkOrders = {
        ...
        "payment-processor-v2.1": [] as {
          order: {
            kind: "payment-processor-v2.1";
            data: Sdk.PaymentProcessorV21.Types.BaseOrder;
          };
          tokenSetId?: string;
          attribute?: {
            collection: string;
        ...
        params.map(async (p) => {
          try {
            if (p.token || p.collection) {
              ...
              const configV3 = await erc721c.v3.getConfigFromDb(contract);
      ...
      await Promise.all(
        params.map(async (params, i) => {
          ...
          const originalParams = JSON.parse(JSON.stringify(params)) as typeof params;

          try {
            // Blacklist checks
            if (collectionId) {
              await checkBlacklistAndFallback(collectionId, params, key);
            }
            if (token) {
              await checkBlacklistAndFallback(token.split(":")[0], params, key);
            }
          } catch (error) {
            return errors.push({
          ...
          try {
            ...
            if (params.checkMakerOutstandingBalance) {
              ...
              switch (params.orderKind) {
                case "seaport-v1.5": {
                  replaceOrderId = (
                    params.options?.["seaport-v1.4"] ??
                    params.options?.["seaport-v1.5"] ??
                    params.options?.[originalParams.orderKind]
                  )?.replaceOrderId;
                  break;
                }
                case "seaport-v1.6": {
                ...
                default:
                  replaceOrderId = (
                    params.options?.[params.orderKind] ?? params.options?.[originalParams.orderKind]
                  )?.replaceOrderId;
                  break;
              }
                ...
                `
                  WITH x AS (
                    ...
                    WHERE token_sets.collection_id = $/collection/
                      ...
                      ${replaceOrderId ? "AND orders.id != $/replaceOrderId/" : ""}
                  )
                  SELECT
            ...
            switch (params.orderKind) {
              ...
              case "seaport-v1.5": {
                ...
                const options = (params.options?.["seaport-v1.4"] ??
                  params.options?.["seaport-v1.5"] ??
                  params.options?.[originalParams.orderKind]) as
                  | {
                      conduitKey?: string;
              ...
              case "alienswap": {
                ...
                const options = (params.options?.[params.orderKind] ??
                  params.options?.[originalParams.orderKind]) as
                  | {
                      useOffChainCancellation?: boolean;
                      replaceOrderId?: string;
              ...
              case "payment-processor-v2.1": {
                ...
                if (token) {
                  const [contract, tokenId] = token.split(":");
                  order = await paymentProcessorV21BuyToken.build({
                    ...params,
                    ...options,
                    maker,
                    contract,
                  ...
                  order = await paymentProcessorV21BuyAttribute.build({
                    ...params,
                    ...options,
                    maker,
                    collection: attribute.collection,
                  ...
                  order = await paymentProcessorV21BuyCollection.build({
                    ...params,
                    ...options,
                    maker,
                    collection,
                ...
                bulkOrders["payment-processor-v2.1"].push({
                  order: {
                    kind: params.orderKind,
                    data: {
                      ...order.params,
      ...
      // Post any payment-processor-v2.1 bulk orders together
      {
        const orders = bulkOrders["payment-processor-v2.1"];
        if (orders.length === 1) {
          const order = new Sdk.PaymentProcessorV21.Order(config.chainId, orders[0].order.data);
          steps[5].items.push({
            status: "incomplete",
            data: {
              sign: order.getSignatureData(),
          ...
          const { signatureData, proofs } = exchange.getBulkSignatureDataWithProofs(
            orders.map((o) => new Sdk.PaymentProcessorV21.Order(config.chainId, o.order.data))
          );
