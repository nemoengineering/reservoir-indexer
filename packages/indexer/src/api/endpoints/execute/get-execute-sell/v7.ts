  ...
  handler: async (request: Request) => {
    ...
    try {
      ...
      const addToPath = async (
        ...
        if (
          ["blur", "sudoswap", "sudoswap-v2", "nftx", "nftx-v3", "zora-v4"].includes(order.kind)
        ) {
      ...
      for (const item of items) {
        ...
        if (item.rawOrder) {
          ...
          if (_.isEmpty(order.kind)) {
            throw getExecuteError("Order kind cannot be empty");
          }
        ...
        if (!item.rawOrder && !item.orderId) {
          ...
          const orderResults = await idb.manyOrNone(
            `
              ...
              WHERE token_sets_tokens.contract = $/contract/
                ...
                ${exactOrderSources.length ? " AND orders.source_id_int IN ($/sources:csv/)" : ""}
                ${item.exclusions?.length ? " AND orders.id NOT IN ($/excludedOrderIds:list/)" : ""}
              ORDER BY ${
          ...
          for (const result of orderResults) {
            ...
            if (
              !["blur", "sudoswap", "sudoswap-v2", "nftx", "nftx-v3", "zora-v4"].includes(
                result.kind
              )
            ) {
      ...
      const forceApprovalProxy =
        Network.CreatorTestnet === config.chainId
          ? false
      ...
      path = path.filter((p) => success[`${p.orderId}:${p.tokenId}`]);
