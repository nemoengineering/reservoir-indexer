...
export const getOrdersBidsV6Options: RouteOptions = {
  ...
  handler: async (request: Request) => {
    ...
    if (query.token && query.endTimestamp) {
      const [contract] = query.token.split(":");
    ...
    let baseQuery = `
        SELECT
          ...
          orders.is_reservoir,
          orders.is_native_off_chain_cancellable,
    ...
    const conditions: string[] =
      query.startTimestamp || query.endTimestamp
        ...
        : [
            `EXISTS (SELECT FROM token_sets WHERE id = orders.token_set_id)`,
            query.token && query.sortBy === "price" ? `orders.side = 'buy'` : `orders.side = 'buy'`,
          ];
    ...
    */
    if (
      ...
      !query.ids &&
      query.status !== "active" &&
      (query.token || query.tokenSetId || query.community || query.collectionsSetId || query.native)
    ) {
    ...
    if (query.token) {
      ...
      conditions.push(`orders.contract = $/tokenContract/`);
    }
    ...
    if (query.collection && !query.attribute) {
      ...
      if (isSharedContract(query.collection)) {
    ...
    if (query.contracts) {
      ...
      if (config.chainId === 1 && query.contracts.length > 1) {
        query.contracts = query.contracts.filter(
          (contract: string) => contract !== "0x790b2cf29ed4f310bf7641f013c65d4560d28371"
        );
      }
      ...
      getJoiOrderObject(
        {
          ...
          isNativeOffChainCancellable: r.is_native_off_chain_cancellable,
