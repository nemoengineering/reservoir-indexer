        ...
        if (
          !items.every(
            (item) =>
              ...
              item.order.kind === "payment-processor-v2.1" ||
              item.order.kind === "alienswap" ||
              item.order.kind === "mintify"
          )
        ) {
          throw Boom.badRequest("Bulk orders are not supported on all requested order kinds");
        }
      }

      // Only ME can create PP orders
      if (
        items.some(
          (item) =>
            item.order.kind === "payment-processor" ||
            item.order.kind === "payment-processor-v2" ||
            item.order.kind === "payment-processor-v2.1"
        )
      ) {
        if (!(await ApiKeyManager.isPaymentProcessorAllowed(apiKey))) {
          throw Boom.badRequest("Order kind not supported");
        }
      }
        ...
        items.map(async (item, i) => {
          ...
          if (signature) {
            try {
              ...
              } else if (bulkData?.kind === "payment-processor-v2.1") {
                const newParams = new Sdk.PaymentProcessorV21.Exchange(
                  config.chainId
                ).encodeBulkSignature(signature, {
                  orderIndex: bulkData.data.orderIndex,
                  proof: bulkData.data.merkleProof,
                });
                order.data = {
                  ...order.data,
                  ...newParams,
          ...
          switch (order.kind) {
            ...
            case "payment-processor-v2.1": {
              ...
              const orderInfo: orders.paymentProcessorV21.OrderInfo = {
                orderParams: order.data,
                metadata: {
                  schema,
                  source,
              ...
              const [result] = await orders.paymentProcessorV21.save([orderInfo]);
