...
export const getUserActivityV6Options: RouteOptions = {
    ...
    schema: Joi.object({
      ...
      activities: Joi.array().items(
        Joi.object({
          ...
          amount: Joi.number().unsafe().allow(null),
  ...
  handler: async (request: Request) => {
    ...
    try {
      ...
      if (query.mintStandard) {
        if (!Array.isArray(query.mintStandard)) {
          mintStandards = [query.mintStandard];
        } else {
          mintStandards = query.mintStandard;
        }
      }

      const limit = query.limit > 20 ? 20 : query.limit;
