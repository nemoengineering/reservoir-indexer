  ...
  public static async getTokens(activities: ActivityDocument[]): Promise<TokenData[]> {
    ...
    if (tokensToFetch.length) {
      ...
      for (const cachedToken of cachedTokens) {
        const cacheKey = `${ActivitiesTokenCache.prefix}:${cachedToken.contract}:${cachedToken.token_id}`;
        await redisMulti.expire(cacheKey, TOKEN_CACHE_TTL);
      }
  ...
  public static async refreshToken(contract: string, tokenId: string, tokenData: any) {
    ...
    await redis.set(
      ...
      JSON.stringify({
        ...
        image_version: tokenData.metadata_version ?? tokenData.image_version,
