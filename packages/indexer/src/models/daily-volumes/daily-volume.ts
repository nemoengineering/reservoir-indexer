...
export class DailyVolume {
  ...
  public static async calculateDay(
    ...
    logger.info(
      "daily-volumes",
      JSON.stringify({
        message: `Calculating daily volumes. startTime=${startTime}, collectionId=${collectionId}, context=${context}`,
        collectionId,
        context,
      })
    );
      ...
      await Promise.all(
        mostRecentTimestampsChunks.map(async (chunk: any) => {
          for (const row of chunk) {
            try {
              let volumeSinceRecent;

              try {
                volumeSinceRecent = {
                  volume_since_recent: await ActivitiesIndex.getCollectionVolumeSinceRecent(
                    row.id,
                    row?.recent_timestamp ? row.recent_timestamp + 24 * 60 * 60 : 0
                  ),
                };
              } catch (error) {
                logger.error(
                  "daily-volumes",
                  JSON.stringify({
                    message: `getCollectionVolumeSinceRecent error. collectionId=${row.id}, error=${error}`,
                    error,
                  })
                );
              }

              if (!volumeSinceRecent) {
                volumeSinceRecent = await redb.oneOrNone(
                  `
      ...
      recentVolumes.forEach((values: any) => {
        queries.push({
          ...
          AND (
            ...
            OR day30_volume IS DISTINCT FROM (CASE WHEN day30_volume < $/volume_since_recent/ THEN $/volume_since_recent/ ELSE day30_volume END)
          )
        `,
          values: values,
        });
