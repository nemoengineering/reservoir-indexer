...
export const refreshOnChainRoyalties = async (collection: string, spec: Spec, useCache = true) => {
  try {
    ...
    const tokenRoyalties = await Promise.all(
      (tokenResults.length ? tokenResults : [{ token_id: "0" }]).map(async (r) =>
        getOnChainRoyalties(token, r.token_id, spec, useCache)
      )
    );
    ...
    if (uniqueRoyalties.length === 1) {
      ...
      try {
        ...
        const randomTokenRoyalties = await getOnChainRoyalties(
          token,
          randomTokenId,
          spec,
          useCache
...
export const getOnChainRoyalties = async (
  ...
  if (useCache) {
    result = await redis.get(cacheKey).then((r) => (r ? (JSON.parse(r) as Royalty[]) : undefined));
  }
