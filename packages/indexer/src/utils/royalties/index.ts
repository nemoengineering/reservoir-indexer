...
export const updateRoyaltySpec = async (
  ...
  if (currentRoyalties) {
    ...
    if (royalties) {
      await FeeRecipients.fixFeeRecipients(royalties.map((r) => r.recipient));
    }
  }
};
...
export const refreshAllRoyaltySpecs = async (
  ...
  if (refreshOnChain) {
    logger.log(
      ...
      "refreshAllRoyaltySpecs",
      JSON.stringify({
        topic: "tokenMetadataIndexing",
        message: `refreshOnChain. collection=${collection}`,
        collection,
...
export const refreshDefaultRoyalties = async (collection: string, context?: string) => {
  const royaltiesResult = await idb.oneOrNone(
    `
      SELECT
        collections.royalties,
    ...
    await orderFixesJob.addToQueue([
      {
        by: "collection",
        data: { collection },
        context: context ?? "refreshDefaultRoyalties",
      },
    ]);
  }

  await FeeRecipients.fixFeeRecipients(defaultRoyalties.map((r) => r.recipient));
};
