...
export const handlePendingMessage = async (message: PendingMessage) => {
  try {
    const { txContents, txHash, time: originatedAt } = message;

    const pendingTokens = await parseTokensFromCalldata(txContents.input);
    if (pendingTokens.length) {
      await addPendingItems(pendingTokens, txHash, originatedAt);
    }
...
export const addPendingItems = async (
  tokens: PendingToken[],
  txHash: string,
  originatedAt: string
) => {
  ...
  for (const { contract, tokenId } of tokens) {
    const pendingItem = {
      ...
      originatedAt: new Date(originatedAt.replace(" ", "T") + "Z").toISOString(),
    } as PendingItem;
