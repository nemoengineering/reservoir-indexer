...
import { getCollectionMints, updateCollectionMintingStatus } from "@/orderbook/mints";
...
export class MintsCheckJob extends AbstractRabbitMqJobHandler {
  ...
  public async addToQueueBatch(mintInfos: MintsCheckJobPayload[]) {
    await this.sendBatch(
      mintInfos.map((mintInfo) => ({ payload: mintInfo, jobId: mintInfo.collection }))
    );
  }
}
