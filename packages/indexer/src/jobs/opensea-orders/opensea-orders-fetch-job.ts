...
export class OpenseaOrdersFetchJob extends AbstractRabbitMqJobHandler {
  ...
  public async process() {
        ...
        if ((error as any).response?.status === 429) {
          ...
          logger.error(
            ...
            JSON.stringify({
              ...
              message: `UnauthorizedError. message=${(error as any).message}, url=${
                (error as any).config?.url
              }`,
              requestHeaders: (error as any).config?.headers,
              responseData: JSON.stringify((error as any).response?.data),
            })
          );
        } else {
          logger.error(
    ...
    for (const collectionOffer of collectionOffers) {
      if (getOpenseaChainName() === collectionOffer.chain) {
