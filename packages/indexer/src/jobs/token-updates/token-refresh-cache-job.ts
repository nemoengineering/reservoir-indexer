...
export default class TokenRefreshCacheJob extends AbstractRabbitMqJobHandler {
  ...
  public async process(payload: TokenRefreshCacheJobPayload) {
    ...
    const floorAsk = await ridb.oneOrNone(
      `
        SELECT
    ...
    if (floorAsk?.id) {
      ...
      logger.debug(
        ...
        JSON.stringify({
          ...
          message: `Simulate response. contract=${contract}, tokenId=${tokenId}, floorAskId=${floorAsk.id}`,
          payload,
    ...
    if (checkTopBid) {
      ...
      const topBid = await ridb.oneOrNone(
        `
          SELECT
            o.id
          FROM orders o
      ...
      if (topBid?.id) {
        ...
        // Simulate
        if ([1, 80094].includes(config.chainId)) {
          await inject({
            method: "POST",
            url: "/management/orders/simulate/v1",
            headers: {
