...
export default class FixTokensMissingCollectionJob extends AbstractRabbitMqJobHandler {
  ...
  public async process() {
    const tokens = await ridb.manyOrNone(
      `
          SELECT contract, token_id, minted_timestamp, created_at
          FROM tokens t 
    ...
    if (tokens) {
      ...
      for (const token of tokens) {
        // If this is not a new token continue
        if (!isAfter(add(new Date(token.created_at), { minutes: 60 }), Date.now())) {
          logger.info(
            this.queueName,
            `not a new token, created at ${token.created_at.toISOString()} no collection for contract ${fromBuffer(
              token.contract
        ...
        if (
          ...
          logger.info(
            this.queueName,
            `no collection for contract ${fromBuffer(token.contract)} tokenId ${
              token.token_id
            } created at ${token.created_at.toISOString()}`
          );
...
if (config.doBackgroundWork && !_.includes([204, 56], config.chainId)) {
  cron.schedule("* * * * *", async () => {
    try {
