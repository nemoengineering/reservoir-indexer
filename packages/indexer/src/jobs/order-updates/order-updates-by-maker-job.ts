...
export default class OrderUpdatesByMakerJob extends AbstractRabbitMqJobHandler {
    ...
    try {
      ...
      switch (data.kind) {
        ...
        case "buy-balance": {
          ...
          if (
            [Network.Blast, Network.Apechain, Network.Berachain].includes(config.chainId) &&
  ...
  public async addToQueue(orderInfos: OrderUpdatesByMakerJobPayload[]) {
    ...
    for (const orderInfosChunk of _.chunk(orderInfos, 500)) {
      await this.sendBatch(orderInfosChunk.map((info) => ({ payload: info, jobId: info.context })));
    }
  }
}
