...
export default class BlurListingsRefreshJob extends AbstractRabbitMqJobHandler {
  ...
  public async addToQueue(collection: string, force = false) {
    await redis.set(`blur-collection-with-listings:${collection}`, "1", "EX", 24 * 3600);
...
if (config.chainId === 1 && config.doBackgroundWork) {
  cron.schedule(
    ...
    "*/10 * * * *",
    async () => {
      if (await acquireLock("blur-listings-refresh-cron-lock", 60 * 5)) {
        const pattern = "blur-collection-with-listings:*";
        let cursor = "0";

        do {
          const [newCursor, keys] = await redis.scan(cursor, "MATCH", pattern, "COUNT", 500);
          cursor = newCursor;

          if (keys.length) {
            await Promise.all(
              keys.map((key) => blurListingsRefreshJob.addToQueue(key.split(":")[1]))
            );
          }
        } while (cursor !== "0");
      }
