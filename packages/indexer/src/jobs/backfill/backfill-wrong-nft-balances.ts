...
if (config.doBackgroundWork) {
  const worker = new Worker(
    QUEUE_NAME,
    async (job) => {
      const { fromBlock, toBlock, contract, user } = job.data;
      ...
      const results = await idb.manyOrNone(
        `
          ...
          WHERE nft_transfer_events.block = $/block/
            ...
            ${contract ? `AND nft_transfer_events.address = $/contract/` : ""}
            ${
              user
                ? `AND (nft_transfer_events.from = $/user/ OR nft_transfer_events.to = $/user/)`
                : ""
            }
          ORDER BY
        ...
        `,
        {
          ...
          user: user ? toBuffer(user) : undefined,
        }
      );
      ...
      if (toBlock > fromBlock) {
        await addToQueue(fromBlock, toBlock - 1, contract, user);
      }
    },
...
export const addToQueue = async (
  ...
  await queue.add(id, { fromBlock, toBlock, contract, user }, { jobId: id });
};
