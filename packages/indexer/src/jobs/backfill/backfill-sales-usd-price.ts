...
import { getCoingeckoNetworkId } from "@/config/network";
...
if (config.doBackgroundWork) {
  const worker = new Worker(
    QUEUE_NAME,
    async (job) => {
      const { startTimestamp, endTimestamp, txHash, logIndex, batchIndex, currency, onlyNulls } =
        job.data;
      const limit = 1000;
      ...
      const results = await idb.manyOrNone(
        `
          ...
          ${currency ? "AND fill_events_2.currency = $/currency/" : ""}
          ${onlyNulls ? "AND fill_events_2.usd_price IS NULL" : ""}
          ORDER BY
        ...
        `,
        {
          ...
          logIndex,
          batchIndex,
          currency: currency ? toBuffer(currency) : "",
        }
      );
      ...
      */
      ...
      // Fix 2: Set the USD price
      {
        ...
        for (const {
          ...
          if (!prices.usdPrice && getCoingeckoNetworkId()) {
