...
export class TokenWebsocketEventsTriggerJob extends AbstractRabbitMqJobHandler {
  ...
  async processCDCEvent(data: TokenCDCEventInfo) {
    ...
    `;
      ...
      if (data.after.metadata) {
        const parsedMetadata = JSON.parse(data.after.metadata);

        if (parsedMetadata.image_original_url) {
          metadata.imageOriginal = parsedMetadata.image_original_url;
        }

        if (parsedMetadata.animation_original_url) {
          metadata.mediaOriginal = parsedMetadata.animation_original_url;
        }

        if (parsedMetadata.image_mime_type) {
          metadata.imageMimeType = parsedMetadata.image_mime_type;
        }

        if (parsedMetadata.media_mime_type) {
          metadata.mediaMimeType = parsedMetadata.media_mime_type;
        }

        if (parsedMetadata.metadata_original_url) {
          metadata.tokenURI = parsedMetadata.metadata_original_url;
        }
      }
  ...
  async processForcedChange(contract: string, tokenId: string, changed: string[]) {
    ...
    try {
      ...
      if (r.metadata?.media_mime_type) {
        metadata.mediaMimeType = r.metadata.media_mime_type;
      }

      if (r.metadata?.metadata_original_url) {
