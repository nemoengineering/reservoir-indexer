...
import { getContractData, getTokenMetadata, publishKafkaEvent } from "./utils";
...
export class SaleWebsocketEventsTriggerQueueJob extends AbstractRabbitMqJobHandler {
  ...
  public async process(payload: SaleWebsocketEventsTriggerQueueJobPayload) {
    ...
    try {
      ...
      if (result.token) {
        let contractKind = r?.contract_kind;
        ...
        (result.token as any).kind = contractKind;
      }
      ...
      logger.error(
        this.queueName,
        JSON.stringify({
          message: `Error processing websocket event. error=${error}`,
          error,
