...
import { FeeRecipientsUpdatedEvent } from "@/pubsub/events/fee-recipients-updated-event";
...
import { CurrencyUpdatedEvent } from "@/pubsub/events/currency-updated-event";

export class PubSub {
  public static getFullChannelName(channelName: Channel) {
    return `${getChainName()}-${channelName}`;
  }
  ...
  public static async subscribe() {
    ...
    redisSubscriber.on("message", async (channel, message) => {
      logger.debug("pubsub", `Received message on channel ${channel}, message = ${message}`);
      ...
      switch (channel) {
        ...
        case PubSub.getFullChannelName(Channel.FeeRecipientsUpdated):
          await FeeRecipientsUpdatedEvent.handleEvent(message);
          break;
        ...
        case PubSub.getFullChannelName(Channel.CurrencyUpdated):
          await CurrencyUpdatedEvent.handleEvent(message);
          break;
      }
    });
  }
...
export class AllChainsPubSub {
  ...
  public static async publish(channel: AllChainsChannel, message = "{}") {
  ...
  public static async subscribe() {
    ...
    allChainsSyncRedisSubscriber.on("message", async (channel, message) => {
      ...
      // Check if the message has scope params and if it has check scope match chain name
      if (
        parsedMessage?.scope &&
        !_.isEmpty(parsedMessage.scope) &&
        parsedMessage.scope !== getChainName()
      ) {
        return;
      }
