...
import { CustomInfo, MintTxSchema } from "@/orderbook/mints/calldata";
  ...
  if (existingCollectionMint) {
    ...
    if (collectionMint.statusReason === "not-yet-started") {
      logger.info(
        ...
        JSON.stringify({
          collection: collectionMint.collection,
          delay: collectionMint.startTime! - now() + 10,
        })
      );
      await mintsRefreshJob.addToQueue(
        {
        ...
        collectionMint.startTime! - now() + 10
      );
    }
...
export const simulateAndUpsertCollectionMints = async (collectionMints: CollectionMint[]) =>
  Promise.all(
    collectionMints.map((collectionMint) => simulateAndUpsertCollectionMint(collectionMint))
  );
...
export const simulateAndUpsertCollectionMint = async (collectionMint: CollectionMint) => {
  ...
  const simulationResult = await simulateCollectionMint(collectionMint, true);

  collectionMint.status = simulationResult ? "open" : "closed";

  return await upsertCollectionMint(collectionMint);
};

export const simulateAndUpsertCollectionPremint = async (
  ...
  const { status, reason } = await getStatus(collectionMint);

  if (status === "open") {
    const simulationResult = await simulateCollectionMint(collectionMint, true, contractKind);

    collectionMint.status = simulationResult ? "open" : "closed";
  } else {
    collectionMint.status = status;
    collectionMint.statusReason = reason;
  }
