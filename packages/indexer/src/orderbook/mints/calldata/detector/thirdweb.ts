...
export const extractByCollectionERC1155 = async (
  ...
  const c = new Contract(
    collection,
    new Interface(["function contractURI() view returns (string)"]),
    baseProvider
  );

  const results: CollectionMint[] = [];
  try {
    const claimConditions: Result[] = [];
    try {
      // Single-phase drops
      ...
      const { currentStartId, count } = await newC.claimCondition(tokenId);
      for (
        let claimConditionId = currentStartId.toNumber();
        claimConditionId < Math.min(currentStartId.toNumber() + count.toNumber(), 10);
        claimConditionId++
      ) {
        claimConditions.push(await newC.getClaimConditionById(tokenId, claimConditionId));
      }
    }

    for (let i = 0; i < claimConditions.length; i++) {
      const claimCondition = claimConditions[i];
