...
const STANDARD = "coinbase-gallery";
...
export const extractByCollection = async (
  ...
  try {
    ...
    if (
      mintingContract.toLowerCase() === Sdk.Coinbase.Addresses.GalleryMintFactory[config.chainId]
    ) {
      ...
      logger.info(
        "coinbase-mint-detection",
        JSON.stringify({
          message: `extractByCollection result. standard=${STANDARD}, collection=${collection}, context=${context}`,
          collection,
          results,
        })
      );
