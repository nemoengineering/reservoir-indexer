...
export type CustomInfo =
  ...
  | (BaseCustomInfo & mints.partydao.Info);
...
export const generateCollectionMintTxData = async (
  ...
  // For `partydao` mints, we need to append the minter field to the calldata
  if (collectionMint.standard === "partydao") {
    data = data + (collectionMint.details.info! as { minter: string }).minter.slice(2);
  }
...
export const refreshMintsForCollection = async (collection: string) => {
  ...
  if (standardResult) {
    switch (standardResult.standard) {
      ...
      case "basepaint":
        await mints.basepaint.refreshByCollection(collection);
        break;

      case "partydao":
        await mints.partydao.refreshByCollection(collection);
        break;

      case "coinbase":
        await mints.coinbase.refreshByCollection(collection);
        break;

      case "coinbase-gallery":
        await mints.coinbaseGallery.refreshByCollection(collection);
        break;
    }
  }
