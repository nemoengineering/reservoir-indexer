...
import { BaseBuildParams } from "@reservoir0x/sdk/dist/payment-processor-v2.1/builders/base";
...
export const getRoyaltiesToBePaid = async (
  ...
  // Royalty ordering: `eip2981` > `pp-registry-backfill` > `onchain` > `opensea` > `custom`
  return (await has("eip2981"))
    ? await get("eip2981")
    : (await has("pp-registry-backfill"))
    ? await get("pp-registry-backfill")
    : (await has("custom"))
    ? await get("custom")
    : (await has("onchain"))
    ? await get("onchain")
...
export const getBuildInfo = async (
  ...
  const nonce = await paymentProcessorV2.getAndIncrementUserNonce(
    orderKind,
    options.maker,
    marketplace
  );
  ...
  const buildParams: BaseBuildParams = {
    protocol:
      collectionResult.kind === "erc721"
        ? Sdk.PaymentProcessorV21.Types.OrderProtocols.ERC721_FILL_OR_KILL
    ...
    masterNonce: await commonHelpers.getMinNonce(orderKind, options.maker),
  };
