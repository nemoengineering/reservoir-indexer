...
import { getRoyaltiesToBePaid } from "@/orderbook/orders/payment-processor-v2.1/build/utils";
import { offChainCheck } from "@/orderbook/orders/payment-processor-v2.1/check";
...
import {
  cosigner,
  isOrderNativeOffChainCancellable,
  saveOffChainCancellations,
} from "@/utils/offchain-cancel";
import { getExternalCosigner } from "@/utils/offchain-cancel/external-cosign";
import { Royalty } from "@/utils/royalties";

export type OrderInfo = {
  orderParams: Sdk.PaymentProcessorV21.Types.BaseOrder;
...
export const save = async (orderInfos: OrderInfo[]): Promise<SaveResult[]> => {
  ...
  const handleOrder = async ({ orderParams, metadata }: OrderInfo) => {
    try {
      const order = new Sdk.PaymentProcessorV21.Order(config.chainId, {
        ...orderParams,
      ...
      if (
        ...
        settings?.paymentSettings === paymentProcessorRegistry.PaymentSettings.PricingConstraints ||
      ...
      } else if (settings?.paymentSettings === paymentProcessorRegistry.PaymentSettings.Paused) {
        return results.push({
          id,
          status: "collection-is-paused",
        });
      }
      ...
      if (side === "buy") {
        ...
        const configV3 = await erc721c.v3.getConfigFromDb(order.params.tokenAddress);
      ...
      // Handle: conduit
      const conduit = Sdk.PaymentProcessorV21.Addresses.Exchange[config.chainId];
      ...
      if (order.isCosignedOrder()) {
        ...
        if (
          ...
          const rawOrder = new Sdk.PaymentProcessorV21.Order(
            config.chainId,
