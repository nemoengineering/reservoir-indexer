...
const prettifyError = (msg: string): PrettyErrorDetails => {
  ...
  switch (true) {
    ...
    case matches("api.opensea.io | 504: Gateway time-out"):
      return {
        message: "OpenSea Error: Gateway time-out",
        status: StatusCode.FAILED_DEPENDENCY,
        code: 4,
      };
    ...
    case error?.source !== undefined: {
      let errorMessage =
        typeof error?.message === "string"
          ? error.message
          : JSON.stringify(error?.message) || "Unknown error";
      if (error.source === "X2Y2" && error?.message?.code) {
        switch (error.message.code) {
          case 1002:
            errorMessage = "User banned from listing (rug/hacker address)";
            break;
          case 1006:
            errorMessage = "A listing order for the NFT already exists";
            break;
          case 2012:
            errorMessage = "WETH balance not enough";
            break;
          case 2014:
            errorMessage = "NFT balance not enough";
            break;
          case 2016:
            errorMessage = "Contract not approved";
            break;
          case 2020:
            errorMessage = "Order already cancelled";
            break;
          case 2021:
            errorMessage = "Order already purchased";
            break;
          case 2028:
            errorMessage = "Contract/NFT is not allowed to trade (rug or hacked NFT)";
            break;
          case 2030:
            errorMessage = "Order already expired";
            break;
          case 2031:
            errorMessage = "Offer already expired";
            break;
          case 3002:
            errorMessage = "Signature error";
            break;
          case 3004:
            errorMessage = "Wrong currency (currently only ETH supported for sell orders)";
            break;
          case 3007:
      ...
      return {
        message: `${error.source} Error: ${errorMessage}`,
        status: StatusCode.FAILED_DEPENDENCY,
        code: 4,
      };
    }
